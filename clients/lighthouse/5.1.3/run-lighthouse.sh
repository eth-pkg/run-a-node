...
if [ "$SHARED_RUN" != "bootnode" ]; then 
    ...
    append_flag "--disable-log-timestamp" "$LIGHTHOUSE_CLI_BOOTNODE_DISABLE_LOG_TIMESTAMP"
    append_flag "--disable-malloc-tuning" "$LIGHTHOUSE_CLI_BOOTNODE_DISABLE_MALLOC_TUNING"
    append_flag "--disable-packet-filter" "$LIGHTHOUSE_CLI_BOOTNODE_DISABLE_PACKET_FILTER"
    append_flag "--enable-enr-auto-update" "$LIGHTHOUSE_CLI_BOOTNODE_ENABLE_ENR_AUTO_UPDATE"
    append_flag "--log-color" "$LIGHTHOUSE_CLI_BOOTNODE_LOG_COLOR"
    append_flag "--logfile-compress" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_COMPRESS"
    append_flag "--logfile-no-restricted-perms" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_NO_RESTRICTED_PERMS"
    append_flag "--version" "$LIGHTHOUSE_CLI_BOOTNODE_VERSION"

    append_option "--testnet-dir" "$LIGHTHOUSE_CLI_BOOTNODE_TESTNET_DIR"
    append_option "--boot-nodes" "$LIGHTHOUSE_CLI_BOOTNODE_BOOT_NODES"
    append_option "--datadir" "$LIGHTHOUSE_CLI_BOOTNODE_DATADIR"
    append_option "--debug-level" "$LIGHTHOUSE_CLI_BOOTNODE_DEBUG_LEVEL"
    append_option "--enr-address" "$LIGHTHOUSE_CLI_BOOTNODE_ENR_ADDRESS"
    append_option "--enr-port" "$LIGHTHOUSE_CLI_BOOTNODE_ENR_PORT"
    append_option "--enr-udp6-port" "$LIGHTHOUSE_CLI_BOOTNODE_ENR_UDP6_PORT"
    append_option "--genesis-state-url" "$LIGHTHOUSE_CLI_BOOTNODE_GENESIS_STATE_URL"
    append_option "--genesis-state-url-timeout" "$LIGHTHOUSE_CLI_BOOTNODE_GENESIS_STATE_URL_TIMEOUT"
    append_option "--listen-address" "$LIGHTHOUSE_CLI_BOOTNODE_LISTEN_ADDRESS"
    append_option "--log-format" "$LIGHTHOUSE_CLI_BOOTNODE_LOG_FORMAT"
    append_option "--logfile" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE"
    append_option "--logfile-debug-level" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_DEBUG_LEVEL"
    append_option "--logfile-format" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_FORMAT"
    append_option "--logfile-max-number" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_MAX_NUMBER"
    append_option "--logfile-max-size" "$LIGHTHOUSE_CLI_BOOTNODE_LOGFILE_MAX_SIZE"
    append_option "--network" "$LIGHTHOUSE_CLI_BOOTNODE_NETWORK"
    append_option "--network-dir" "$LIGHTHOUSE_CLI_BOOTNODE_NETWORK_DIR"
    append_option "--port" "$LIGHTHOUSE_CLI_BOOTNODE_PORT"
    append_option "--port6" "$LIGHTHOUSE_CLI_BOOTNODE_PORT6"
    append_option "--safe-slots-to-import-optimistically" "$LIGHTHOUSE_CLI_BOOTNODE_SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY"
    append_option "--terminal-block-hash-epoch-override" "$LIGHTHOUSE_CLI_BOOTNODE_TERMINAL_BLOCK_HASH_EPOCH_OVERRIDE"
    append_option "--terminal-block-hash-override" "$LIGHTHOUSE_CLI_BOOTNODE_TERMINAL_BLOCK_HASH_OVERRIDE"
    append_option "--terminal-total-difficulty-override" "$LIGHTHOUSE_CLI_BOOTNODE_TERMINAL_TOTAL_DIFFICULTY_OVERRIDE"
fi 
...
if [ "$SHARED_RUN" != "bootnode" ]; then 
    ...
    echo "Starting lighthouse boot_node $OPTIONS"
    lighthouse beacon_node $OPTIONS
fi 